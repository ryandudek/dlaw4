...
//--------------------------------------- Do Desktop Stuff
  ...
  function wrapForCollapse(){
    ...
    $.each(h2Array, function(index, val) {
      that = this;
      $(val).click(function(event) {
        
        $(h2CollapsibleArray[index]).slideToggle();     
        $(h2Array[index]).toggleClass('closeed-header');
        if (!$(h2Array[index]).hasClass('closeed-header')) {
          $('html, body').animate({
              scrollTop: $(h2Array[index]).offset().top
          }, 1000);
        };
